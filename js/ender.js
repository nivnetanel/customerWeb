...
!(function (a, b) {
    ...
    var b = a.document,
      ...
      m = function () {
        var b = k,
          ...
          h,
          i;
        for (; d < c; d++) (e = b[d]), (f = e.href), (h = e.media), (i = e.rel && e.rel.toLowerCase() === "stylesheet"), !!f && i && !g[f] && (e.styleSheet && e.styleSheet.rawCssText ? (o(e.styleSheet.rawCssText, f, h), (g[f] = !0)) : ((!/^([a-zA-Z:]*\/\/)/.test(f) && !j) || f.replace(RegExp.$1, "").split("/")[0] === a.location.host) && l.push({ href: f, media: h }));
        n();
      },
      n = function () {
        if (l.length) {
      ...
      o = function (a, b, c) {
        ...
        for (; j < g; j++) {
          ...
          for (; k < o; k++) (m = n[k]), d.push({ media: (m.split("(")[0].match(/(only\s+)?([a-zA-Z]+)\s?/) && RegExp.$2) || "all", rules: e.length - 1, hasquery: m.indexOf("(") > -1, minw: m.match(/\(min\-width:[\s]*([\s]*[0-9\.]+)(px|em)[\s]*\)/) && parseFloat(RegExp.$1) + (RegExp.$2 || ""), maxw: m.match(/\(max\-width:[\s]*([\s]*[0-9\.]+)(px|em)[\s]*\)/) && parseFloat(RegExp.$1) + (RegExp.$2 || "") });
        }
        t();
      },
      p,
  ...
  (function (a) {
    ...
    for (var h = 0; h < f.length; h++)
      f[h].innerHTML = f[h].innerHTML.replace(/\u00ab(.+?)\u00b7(.+?)\u00b7\1\u00bb/g, function (a, b, c) {
